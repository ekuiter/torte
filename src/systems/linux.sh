...
# Linux is a fairly complicated system, also there is no predefined define-system here
# instead all the functions are handcrafted (e.g., to account for CPU architectures)

# by default, we work on a rewritten (i.e., transformed) repository of Linux that has some useful properties
...
# after cloning, execute all transformations requested by the first call to add-linux-system
...
tag-old-releases-linux() {
    ...
    tag-old-releases-linux-helper https://mirrors.edge.kernel.org/pub/linux/kernel/v2.5/ 2.5.45
    tag-old-releases-linux-helper https://mirrors.edge.kernel.org/pub/linux/kernel/v2.6/ 2.6.0 2.6.12
...
# downloads old Linux releases from the given webpage and tags them in the Git repository
tag-old-releases-linux-helper(base_uri, start_inclusive=, end_exclusive=) {
    local revisions
