...
SE=$(dirname "$0")/SatELite-2011           #set this to the executable of SatELite
RS=$(dirname "$0")/glucose-2011              #set this to the executable of RSat
INPUT=$1;

cleanup () {
  echo rm -f $TMP.cnf $TMP.vmap $TMP.elim $TMP.result
}

trap "cleanup" 2 3 9 11 15
