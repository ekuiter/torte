...
kconfig-checkout() {
    ...
    local kconfig_binding_files=$(kconfig-binding-files $kconfig_binding_files_spec)
    local kconfig_binding_directory=$(kconfig-binding-directory $kconfig_binding_files_spec)

    pushd $(input-directory)/$system
    git-checkout $revision $PWD
    kconfig-post-checkout-hook $system $revision
    if [[ ! -z $kconfig_binding_files_spec ]]; then
...
compile-kconfig-binding() (
    ...
    local kconfig_binding_files=$(kconfig-binding-files $kconfig_binding_files_spec)
    local kconfig_binding_directory=$(kconfig-binding-directory $kconfig_binding_files_spec)
    local kconfig_binding_output_file=$(output-directory)/$kconfig_bindings_output_directory/$system/$revision.$kconfig_binding_name
...
extract-kconfig-model() (
    ...
    if [[ -f $(output-directory)/$kconfig_models_output_directory/$system/$revision,*,$extractor.model ]]; then
    ...
    while [[ $i -ne $N ]]; do
        i=$(($i+1))
        local model="$(output-directory)/$kconfig_models_output_directory/$system/$revision,$i,$extractor.model"
